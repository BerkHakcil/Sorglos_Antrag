...
8. **Migrations are the single source of truth.** Schema changes go through SQL migrations in `supabase/migrations`, committed to the repo. Do not rely on ad-hoc Supabase dashboard edits. Provide a reproducible seed.
   - **Migration-before-code ordering (hard rule).** Code that references a new DB column or table must **never** deploy before the migration that creates it. The order is: **migration pushed and verified on prod FIRST, then the dependent code deploy.** If a single commit contains both, the commit waits until the migration is applied. (Pushing code first means every request between deploy and migration hits a column that does not exist — a live outage of that feature. Adding a **row** to an existing table is the benign case and may deploy first, since missing rows degrade to empty strings by design; adding a **column** is not.)
