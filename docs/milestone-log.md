...
- **Fresh-case required-question count (progress denominator):** **58** for a single applicant (Tier 7 made `power_of_attorney` optional, dropping it from the previous 59; before answering `marital_status` — selecting a spouse status reveals the ~30-question spouse section and it rises, e.g. to ~91 for "verheiratet").
...
1. **Migrations only — no dashboard/Studio content edits.** All content/schema changes go through dated `supabase/migrations/*.sql` applied with `supabase db push`. If Studio is _ever_ used for something urgent, **immediately** backfill the migration history (`supabase migration repair --status applied <version>`) — the Tier 2 discovery showed that skipping this leaves prod untracked and creates repo↔DB drift. Migrations are the single source of truth (CLAUDE.md #8).
