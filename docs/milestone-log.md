...
## Current state snapshot (as of 2026-08-26)
...
## Fallback document list: Line-A purge + banner removal — ✅ SHIPPED 2026-08-26, live-verified same day
...
fallback infobox is removed. Gate 2 executed same day: founder pushed
migration `20260826000001` (`supabase db push`), POST-mode verification all
green, branch fast-forward merged to `main` (`c013233`) and deployed.

**Live verification evidence (2026-08-26, after deploy):**
`scripts/verify-fallback-doclist.mjs` POST mode **ALL CHECKS PASSED, 0
warnings** — run by the founder after the push and independently re-run: the
config row is exactly the Line-A trio and each id is an active default-office
rule; Pankow (49) and Essen (56) own-office resolution byte-identical with
and without the exclusion row; every Phase-1 §4 Line-A prediction landed
exactly per case — `52e364f1` **14 slots / missing 0** with its 3 trio files
flagged `not_required` (complete case stayed complete), `78293a6c` **18/5**
with 1 `not_required`, Essen `ecdf545d` **13/7 untouched**, fresh-shape
fallback cases 9-10 slots, no Line-A slot served anywhere, no missing count
increased. Prod e2e `fallback-notice.spec.ts` **3/3 in 37.6s**: F1 (21682
fallback) — no banner, no "(letzte …)" suffix, trio ABSENT from the rendered
checklist; F2 (Pankow 13187) and F3 (Essen 45127) — own lists with suffix
intact, no banner; all throwaway users deleted.
