...
| **E-7** | **Polish + a11y sweep.** 404/error pages, focus states, touch-target audit, contrast re-verification on the built pages, mobile pass at 375 px on every screen. **Sweep list: replace the two remaining `waitForLoadState('networkidle')` calls in `scripts/ui-gallery.mjs` with deterministic waits** — the specs were cleared in `53fdf73` but the capture script still carries the fragile primitive (it works today; it is the same failure mode waiting to happen, and a hung gallery run blocks a sub-phase gate). | various                                                        | low                                                     |
