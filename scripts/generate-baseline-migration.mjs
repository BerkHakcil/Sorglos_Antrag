...
for (const [label, err] of [
  ['category', catErr], ['question_group', grpErr],
  ['question', qErr],   ['question_option', optErr],
]) {
  if (err) { console.error(`Error fetching ${label}:`, err.message); process.exit(1) }
}
...
for (const cat of categories) {
  ...
  if (!seed) {
    ...
    out.push(`VALUES (${esc(cat.id)}, ${esc(cat.questionnaire_id)}, ${esc(cat.key)}, ${cat.sort_order}, ${esc(cat.label_de)})`)
    out.push(`ON CONFLICT (id) DO UPDATE SET key = EXCLUDED.key, label_de = EXCLUDED.label_de, sort_order = EXCLUDED.sort_order;`)
    labelChanges++
    console.log(`  New category: ${cat.key} = "${cat.label_de}"`)
  } else if (seed.key !== cat.key || seed.label_de !== cat.label_de) {
    out.push(`UPDATE public.category SET key = ${esc(cat.key)}, label_de = ${esc(cat.label_de)} WHERE id = ${esc(cat.id)};`)
    labelChanges++
...
const KNOWN_SEED_KEYS = [
  'last_name','birth_name','first_name','birthdate','city_of_birth','district_of_birth',
  'country_of_birth','gender','marital_status','marital_status_since','citizenship',
  'issuer_of_id','id_expiry_date','prior_social_aid','prior_social_aid_until',
  'prior_social_aid_issuer','prior_social_aid_reference_id','power_of_attorney',
  'special_origin_rights','special_origin_rights_issued','special_origin_rights_issued_by',
  'disability_card','disablity_card_application','disability_card_expiry',
  'disability_card_markers','health_insurance','health_insurance_type','care_level',
  'in_facility_since','prior_social_service_applications','last_residence_street',
  'last_residence_city','last_residence_plz','berlin_since','berlin_district_since',
  'apartment_ownership','landlord_name_and_address','rent_total','rent_heating',
  'rent_warm_water','rent_paid_until','rent_debt','rent_contract_termination_yes_no',
  'rent_contract_terminated_by','child_first_name','child_last_name','child_birth_name',
  'child_birth_date','child_marital_status','child_family_tie','child_profession',
  'child_address','pension_type','pension_amount','pension_id','pension_issuer',
  'wohngeld_yes_no','wohngeld_amount','wohngeld_id','other_income','other_income_type',
  'other_income_amount','govermental_employee','health_insurance_amount',
  'care_insurance_amount','general_liablity_insurance_yes_no',
  'general_liablity_insurance_provider','general_liability_amount','life_insurance',
  'life_insurance_monthly_amount','life_insurance_total_amount','life_insurance_name',
  'life_insurance_number','funeral_insurance_yes_no','funeral_insurance_amount',
  'funeral_insurance_detail','bank_giro','bank_giro_blz','bank_giro_iban',
  'bank_giro_amount','bank_savings_account_yes_no','bank_savings_account_amount',
  'bank_savings_iban','bank_additional_account_yes_no','bank_additional_name',
  'bank_additional_iban','bank_additional_amount','cash_savings','automobile_owner',
  'automobile_numbers_plate','automobile_type','automobile_year','automobile_holder',
  'property_yes_no','property_address','property_usage','property_size',
  'additional_wealth_yes_no','additional_wealth_type','additional_wealth_amount',
  'costly_diet',
  // spouse section
  'spouse_last_name','spouse_birth_name','spouse_first_name','spouse_birthdate',
  'spouse_city_of_birth','spouse_district_of_birth','spouse_country_of_birth',
  'spouse_gender','spouse_citizenship','spouse_issuer_of_id','spouse_id_expiry_date',
  'spouse_prior_social_aid','spouse_prior_social_aid_until','spouse_prior_social_aid_issuer',
  'spouse_prior_social_aid_reference_id','spouse_power_of_attorney',
  'spouse_special_origin_rights','spouse_special_origin_rights_issued',
  'spouse_special_origin_rights_issued_by','spouse_disability_card',
  'spouse_disability_card_application','spouse_disability_card_expiry',
  'spouse_disability_card_markers','spouse_health_insurance','spouse_health_insurance_type',
  'spouse_care_level','spouse_in_facility_yes_no','spouse_in_facility_since',
  'spouse_prior_social_service_applications','spouse_pension_type','spouse_pension_amount',
  'spouse_pension_id','spouse_pension_issuer','spouse_wohngeld_yes_no',
  'spouse_wohngeld_amount','spouse_wohngeld_id','spouse_other_income',
  'spouse_other_income_type','spouse_other_income_amount','spouse_health_insurance_amount',
  'spouse_care_insurance_amount','spouse_general_liablity_insurance_yes_no',
  'spouse_general_liablity_insurance_provider','spouse_general_liability_amount',
  'spouse_life_insurance','spouse_life_insurance_amount','spouse_bank_savings_account_amount',
  'spouse_bank_account_amount','spouse_automobile_owner','spouse_automobile_numbers_plate',
  'spouse_automobile_type','spouse_automobile_year','spouse_automobile_holder',
  'spouse_property_yes_no','spouse_additional_wealth_yes_no','spouse_additional_wealth_type',
